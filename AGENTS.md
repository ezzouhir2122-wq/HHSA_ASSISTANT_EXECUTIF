...
# AGENTS.md — Executive Assistant Command Centre
...
**MCP actif :** Zapier — HTTP transport, token dans `.Codex/settings.local.json`. Automatisation directe depuis Codex.
...
| `code-review` *(agent)* | "révise mon code" / "code review" / `/code-review` | Sous-agent isolé → git diff → analyse bugs/qualité/sécu/lisibilité → rapport dans live/reviews/. Déclenché via l'agent `.Codex/agents/code-reviewer.md` |
...
`.Codex/rules/` auto-chargés · `.Codex/skills/` construits à la demande · `archive/` rien n'est supprimé
